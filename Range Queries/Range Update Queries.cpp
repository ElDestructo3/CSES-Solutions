...
void solve(int t) {
    ...
    while (q--) {
        ...
        cin >> type;
        if (type == 1) {
            int left, right, val_increase;
            cin >> left >> right >> val_increase;
            if (right < n) {
                segTree.update(right, b[right] - val_increase);
                b[right] -= val_increase;
            }
            segTree.update(left - 1, b[left - 1] + val_increase);
            b[left - 1] += val_increase;
        }
        else {
            int index;
            cin >> index;
            cout << segTree.apply_range(0, index - 1) << endl;
            cout.flush();
        }
    } 
